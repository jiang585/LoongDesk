...
function dropText(event: DragEvent | globalThis.DragEvent) {
...
export function DropCapture({ children, compact = false }: { children: ReactNode; compact?: boolean }) {
  ...
  const capture = async (text: string, sourceType: ConcernSourceType) => {
    if (!text.trim()) return
  ...
  const captureFiles = async (files: FileList | File[]) => {
